// FlameWrap 火焰特效的 Web Worker 渲染端。
// 单 pass WebGL2 火焰着色器 + 定时渲染循环,跑在独立线程,与主线程的
// React 渲染(SSE 消息流式更新)完全解耦。主线程只负责:几何测量
// (输入框相对画布的矩形,节流)、layoutsubtree 内容快照(节流)、
// 可见性/动效偏好转发,渲染循环本身不占主线程。
...
export type FlameWorkerRequest =
  | {
      type: "init";
...
export type FlameWorkerResponse = {
  type: "ready";
  ok: boolean;
};
...
let config: Required<FlameWrapOptions> = { ...DEFAULTS };
...
const smooth = {
  ...
  color: [DEFAULTS.color[0], DEFAULTS.color[1], DEFAULTS.color[2]] as [
    number,
    number,
    number,
  ],
...
function post(message: FlameWorkerResponse) {
  (self as unknown as { postMessage(m: unknown): void }).postMessage(message);
}

function render() {
  if (!gl) return;
  if (contentDirty && pendingContent) {
  ...
  gl.uniform2f(uniforms.uResolution, cssWidth * dpr, cssHeight * dpr);
  ...
  gl.uniform1f(uniforms.uHasContent, contentSupported ? 1 : 0);
  gl.bindFramebuffer(gl.FRAMEBUFFER, null);
  gl.viewport(0, 0, Math.round(cssWidth * dpr), Math.round(cssHeight * dpr));
...
function initState(msg: Extract<FlameWorkerRequest, { type: "init" }>) {
  config = { ...DEFAULTS, ...msg.options };
  cssWidth = Math.max(msg.cssWidth, 1);
  cssHeight = Math.max(msg.cssHeight, 1);
  dpr = msg.width / Math.max(cssWidth, 1);
  rect = { ...msg.rect };
  contentSupported = msg.contentSupported;
  smooth.height = config.height;
  smooth.intensity = config.intensity;
  smooth.color = [config.color[0], config.color[1], config.color[2]];
  ...
  if (!gl.getShaderParameter(fragmentShader, gl.COMPILE_STATUS)) {
    console.error("FlameWrap shader error:", gl.getShaderInfoLog(fragmentShader));
  }
...
function handleGeometry(msg: Extract<FlameWorkerRequest, { type: "geometry" }>) {
  cssWidth = Math.max(msg.cssWidth, 1);
  cssHeight = Math.max(msg.cssHeight, 1);
  dpr = msg.width / Math.max(cssWidth, 1);
  rect = { ...msg.rect };
...
function handleContent(msg: Extract<FlameWorkerRequest, { type: "content" }>) {
  if (destroyed) return;
...
function handleSetOptions(msg: Extract<FlameWorkerRequest, { type: "setOptions" }>) {
  let changed = false;
  for (const [key, value] of Object.entries(msg.options)) {
...
self.onmessage = (event: MessageEvent<FlameWorkerRequest>) => {
