        ...
        while let Some(event_result) = event_source.next().await {
            match event_result {
                ...
                Ok(Event::Message(message)) => {
                    ...
                    // 原实现只 continue,turn 要等 HTTP 连接真正关闭才结束;
                    // 部分网关/中转在 [DONE] 后保持连接不关,导致流挂起,
                    // 前端最后一条消息长时间等待。收到哨兵即按流结束处理,
                    // 跳出循环后统一走 close + 收尾(工具调用/usage)路径。
                    if message.data == "[DONE]" {
                        tracing::debug!("SSE stream received [DONE] sentinel, ending stream");
                        break;
                    }
...
mod tests {
    ...
    // combo patch 回归测试:验证 `data: [DONE]` 哨兵能立即终结流,
    // 即使底层 HTTP 传输保持连接不关闭(部分网关/中转的行为)。
    // 修复前该用例会在 event_source.next() 上永久挂起,超时断言失败。
    mod done_sentinel {
        use super::super::test_support::sse_bytes_from_data_lines;
        use crate::providers::internal::openai_chat_completions_compatible::{
            send_compatible_streaming_request, CompatibleChoice, CompatibleChoiceData,
            CompatibleChunk, CompatibleFinishReason, CompatibleStreamProfile,
        ...
        /// 把每条 data 解析成文本增量 chunk 的最小 profile。
        struct PlainTextProfile;

        impl CompatibleStreamProfile for PlainTextProfile {
            ...
            ) -> Result<Option<CompatibleChunk<Self::Usage, Self::Detail>>, CompletionError>
            {
                Ok(Some(CompatibleChunk {
                    ...
                    response_model: None,
                    usage: None,
                    choice: Some(CompatibleChoice::from(CompatibleChoiceData::<
                        CompatibleToolCallChunk,
                        (),
                    > {
        ...
        async fn done_sentinel_ends_stream_even_when_transport_stays_open() {
            let client = HangingOpenClient {
                sse_bytes: sse_bytes_from_data_lines(["hello", "[DONE]"]),
            };
            ...
            let mut stream = send_compatible_streaming_request(client, req, PlainTextProfile)
                .await
                .expect("stream should start");
            ...
            assert!(saw_final, "final response should still be yielded");
        }
    }
}
