    ...
    fn interpret(&mut self, event: Self::Event, out: &mut AdapterOutput<Self::Response>) {
        let chunk = match event {
            CompatEvent::Done => {
                ...
                // combo patch: OpenAI 协议规定 `data: [DONE]` 即流终止。
                // 原实现只置 saw_terminal 标志就 return,driver 会继续等
                // transport 的下一帧——turn 要等 HTTP 连接真正关闭才结束;
                // 部分网关/中转在 [DONE] 后保持连接不关,导致流挂起,
                // 前端最后一条消息长时间等待。收到哨兵即按流结束处理:
                // 立即走 finish 收尾路径(工具调用 flush + usage + 终态),
                // driver 检测到 FinalResponse 后停止消费,不等 EOF。
                // (0.41 补丁的等价移植:该路径在 0.42 的 WireAdapter 重构后
                // 仍依赖 transport EOF,问题未在 rig 上游修复。)
...
mod tests {
    ...
    // combo patch 回归测试(0.42 WireAdapter 重构后的等价移植):验证
    // `data: [DONE]` 哨兵能立即终结流,即使底层 HTTP 传输保持连接不关闭
    // (部分网关/中转的行为)。0.42 的 CompatAdapter 收到 Done 只置
    // saw_terminal 标志,driver 仍会继续等 transport 的下一帧——turn 要等
    // HTTP 连接真正关闭才结束;本补丁在 Done 分支直接调用 finish 产出终态。
    // 修复前该用例会在 stream.next() 上永久挂起,超时断言失败。
    mod done_sentinel {
        use super::super::test_support::sse_bytes_from_data_lines;
        ...
        /// 发出一段 SSE 字节后永远 Pending(模拟网关不关连接)的字节流。
        struct HangingTailStream {
            bytes: Option<Bytes>,
        }

        impl futures::Stream for HangingTailStream {
        ...
        struct HangingOpenClient {
            sse_bytes: Bytes,
        }

        impl HttpClientExt for HangingOpenClient {
            fn send<T, U>(
                &self,
                _req: http::Request<T>,
            ) -> impl Future<Output = http_client::Result<http::Response<http_client::LazyBody<U>>>> + crate::wasm_compat::WasmCompatSend + 'static
            where
            ...
            fn send_streaming<T>(
                &self,
                _req: http::Request<T>,
            ) -> impl Future<Output = http_client::Result<http_client::StreamingResponse>> + crate::wasm_compat::WasmCompatSend
            where
                T: Into<Bytes> + crate::wasm_compat::WasmCompatSend,
            {
                let sse_bytes = self.sse_bytes.clone();
                async move {
                    let byte_stream = HangingTailStream {
                        bytes: Some(sse_bytes),
                    };
        ...
        #[tokio::test]
        async fn done_sentinel_ends_stream_even_when_transport_stays_open() {
            let client = HangingOpenClient {
                // "combined" 是有效文本/推理 chunk(无 finish_reason),[DONE] 触发
                // 补丁的 finish 收尾:流应产出 FinalResponse 并结束,不等 transport。
                sse_bytes: sse_bytes_from_data_lines(["combined", "[DONE]"]),
            };
            ...
            // 传输永不关闭:若 [DONE] 未按流结束处理,这里会一直等待直到超时。
            let drained = tokio::time::timeout(std::time::Duration::from_secs(2), async {
                let mut saw_final = false;
                while let Some(item) = stream.next().await {
                    match item.expect("stream item should be ok") {
                        crate::streaming::StreamedAssistantContent::Final(_) => {
                            saw_final = true;
                        }
                        _ => {}
                    }
            ...
            let saw_final =
                drained.expect("stream should end on [DONE] without waiting for connection close");
            assert!(saw_final, "final response should still be yielded after [DONE]");
        }
    }
}
